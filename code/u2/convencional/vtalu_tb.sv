...
`endif
   ...
   // coverage counts them anyway -- passes green and has measured nothing.
   final begin
      if (enviadas != chequeadas)
        $error ("sent %0d operations, the scoreboard checked %0d", enviadas, chequeadas);
      else
        $display ("%0d operations sent, %0d checked", enviadas, chequeadas);
   end
endmodule : top