...
**Quince**, repartidos por día. Cada uno se corrige solo: el `run.sh` falla hasta
que lo resolvés.
...
corrector va por etapas —monitor, driver, scoreboard, cobertura— y cada una
imprime su `STAGE N OK`, así que se puede terminar de a una.
...
Once usan UVM: la primera compilación tarda ~1 min 30 en una laptop de 12 cores
...
porque las ondas *son* el ejercicio. Los cinco que randomizan con constraints
—`d5b`, `d6-bins`, `d6-semillas`, `d7-final` y `d8-fifo`— necesitan además
...
Los cuatro del día 6, los dos del día 7 y los dos del día 8 compilan un testbench
