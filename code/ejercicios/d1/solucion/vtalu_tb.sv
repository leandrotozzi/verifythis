...
module top;
   ...
   covergroup op_cov;
      coverpoint op_set {
         bins single_cycle[] = {[add_op : xor_op], shr_op, rst_op,no_op};
...
`endif
   ...
   always @(posedge done) begin : scoreboard
      ...
      case (op_set)
        ...
        // El DUT desplaza por B[2:0], no por B entero: con B = 8'h20 el
        // hardware corre 0 lugares, no 32. El modelo tiene que hacer lo mismo.
        shr_op: predicted_result = 16'(A) >> B[2:0];
      endcase // case (op_set)
   ...
   // --- correccion del ejercicio (no hace falta que la toques) ---------------
   // Un $error en Verilator aborta la simulacion, asi que un resultado que no
   // coincida se ve solo: lo unico que hay que contar son los shifts.
   int shifts = 0;

   final begin
      if (shifts == 0)
        $display("EJERCICIO INCOMPLETO: el TB nunca mando un shift (mira get_op)");
      else
        $display("EJERCICIO OK: %0d shifts chequeados, 0 errores", shifts);
   end
