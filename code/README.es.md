...
**La única excepción son los `README`**, que son el enunciado del ejercicio y
no documentación del código: ahí sí hay dos, `README.md` en inglés y
`README.es.md` al lado. El inglés es el que se llama `README.md` porque es el
único que GitHub renderiza al entrar a un directorio, y el que cae acá desde una
búsqueda casi siempre lee en inglés — la misma razón por la que el README de la
raíz va al revés que el resto del repo. Se escribe en castellano igual: la fuente
es el `.es.md` y `npm run check` verifica que el par no se haya desincronizado
— ver `tools/lint-i18n.mjs`.
