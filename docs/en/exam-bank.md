...
| 5 | 1 | Interfaces and BFM | **a** | **It stops talking in signals** — the BFM translates *one operation* into *a handshake of signals*. The tester never touches a wire again; the scoreboard and the coverage keep reading them, through the interface, until a monitor sends them transactions. It is the first step towards UVM. |
