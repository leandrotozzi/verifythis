...
![Two sibling testers repeating the same run_phase](res/diagrams/en/env_intract_sol_1.svg)
<!-- .element: class="grande" -->
...
![base_tester abstract with get_op and get_data pure virtual](res/diagrams/en/env_using_virtual_class.svg)
<!-- .element: class="grande" -->
...
![The three-storey hierarchy: base_tester, random_tester and add_tester](res/diagrams/en/env_adaptable_sol.svg)
<!-- .element: class="grande" -->
...
![Two parallel hierarchies: the tests on the left and the env on the right](res/diagrams/en/env_struct_level.svg)
<!-- .element: class="grande" -->
...
![The formula class::type_id::create(name, this), piece by piece](res/diagrams/en/env_uvm_incantation.svg)
<!-- .element: class="grande" -->
...
![set_type_override_by_type: the factory returns add_tester where base_tester is asked for](res/diagrams/en/env_uvm_incantation_2.svg)
<!-- .element: class="grande" -->
...
- `factory.print(0)` is the counterpart of `+TOPOLOGY`: one says what you
  **asked for**, the other what **came out**. When they do not match, the bug is in between
- With `*` it works: `"*.env_h.t4_h"` applies. It is the way round when the test does not know where
...
The practical rule: the path is the one `+TOPOLOGY` prints, without the `<unnamed>` at the
