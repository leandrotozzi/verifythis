...
![The same move: tangled and simple](res/funs/en/simple-es-dificil.svg)
...
#### *The problem: two tests, the same `build_phase()` twice*

![random_test and add_test write the same build_phase()](res/diagrams/en/env_intract_sol_1.svg)
<!-- .element: class="grande" -->

- Today there are two tests: `random_test` builds a `random_tester`, `add_test` an
  `add_tester`. Everything else —coverage and scoreboard— they build the same way
- `add_test` **inherits** from `random_test` and **rewrites its whole
  `build_phase()`**: the three `new()` calls are identical and written twice
- The testers, on the other hand, duplicate nothing: `add_tester extends
  random_tester` and only overrides `get_op()`. The `run_phase()` is written **once**,
  in `random_tester`, and it is inherited
- Which means: the inheritance ladder is already solved well below, in the testers,
  and solved badly above, in the tests. The next two slides show how it came out
  below; the rest of the section applies it above
- Duplicating the `build_phase` is not ugly, it is expensive: ten tests are ten
  copies, and one change in the structure is ten edits

Note:
This is the problem slide, and it is worth not jumping ahead to the solution: let the
duplication be seen first, and let it be seen **where it is**. It is not in the
testers —those already inherit properly— but in the tests, which is where nobody
looks for it.
The question that orders the next two slides: which part of a tester is *the test*,
and which part is
...
![abstract base_tester with a shared run_phase() and pure virtual methods](res/diagrams/en/env_using_virtual_class.svg)
<!-- .element: class="grande" -->
...
![random_tester as the base and add_tester inheriting from it](res/diagrams/en/env_adaptable_sol.svg)
<!-- .element: class="grande" -->
...
![The class diagram before and after splitting structure from stimulus with uvm_env](res/diagrams/en/env_struct_level.svg)
<!-- .element: class="grande" -->
...
![Anatomy of the line type::type_id::create(name, this)](res/diagrams/en/env_uvm_incantation.svg)
<!-- .element: class="grande" -->
...
- The misspelt class and the one that does not exist show at **compile time**. The one
  that forgot the `` `uvm_component_utils `` only if **no ancestor** is registered: if it
  extends a registered one, the `type_id` is **inherited** and there is no error
...
this"*.
The last bullet is the one that is **not** reassuring, and it should be said that way.
If `base_tester` was not registered and nothing above it was either,
`base_tester::type_id` does not exist and it does not compile: that is the good case.
But in this very section's hierarchy —the three of them registered— forgetting the
macro in `add_tester` **breaks nothing**: the `type_id` and the `get_type()` are
inherited from `random_tester`, the testbench compiles clean, and the `set_type_override`
of `add_test` ends up registering `random_tester`. Measured:
`add_tester::get_type().get_type_name()` returns `random_tester`, the env builds a
`random_tester`, and the test runs with `UVM_ERROR : 0`. It belongs to the "does not
break, lies" family of the silent-traps appendix: the only case where it does fail to
compile is if the handle is declared of the derived type, and here the handle is
...
![Anatomy of the factory override with set_type_override()](res/diagrams/en/env_uvm_incantation_2.svg)
<!-- .element: class="grande" -->
...
- `set_inst_override` changes **one path**, not a type. And where the two coincide,
  the instance one wins
- The path is **absolute and starts at `uvm_test_top`**. Measured: `"env_h.t3_h"` gets
  registered all the same, gives no error, and `t3_h` comes out an `add_tester`
- `factory.print(0)` is the counterpart of `uvm_top.print_topology()`: one says what you
  **asked for**, the other what **came out**. When they do not match, the bug is in between
- With `*` it works: `"*.env_h.t4_h"` applies. It is the workaround when the test does not know where
  it is going to be instantiated
