...
- Write the directed case that closes it. But **ask for it with
  `randomize() with {}`**, not by assigning the fields, which is what this
  section's `tester` does three lines above
...
bin on their own with the 60 random ones — and that is the exercise day 7 starts
with.
