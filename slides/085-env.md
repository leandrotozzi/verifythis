...
Y el enganche hacia atrás: esto es polimorfismo usada en serio. `pure virtual` en
...
- Dos jerarquías paralelas, y ése es toda la sección: a la izquierda los tests
...
Note:
La macro es lo que reemplaza al `case` de la fábrica escrita a mano de la factory: cada clase se anota sola en el diccionario, y agregar un tipo no obliga a
editar la fábrica.
...
- Los tres argumentos son los de siempre: el **nombre** que va a salir en la
...
- Todos los Tests hace un override de base_tester en función del tester que necesitan para sus estímulos
