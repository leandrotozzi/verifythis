...
- [x] Los monitores y los analysis ports; el sequencer y el driver quedan en `null`
- [ ] Sólo el sequencer, para poder recibir sequences de otro agent

> **Los monitores, siempre** — mirar nunca es opcional: un agent pasivo sigue alimentando scoreboard y cobertura. Lo que se saltea es lo que *maneja* la interface, porque ahí ya hay otro manejándola.
...
- [ ] El item se descarta y el scoreboard reporta un mismatch
- [x] La sequence se queda esperando en `finish_item()` y la simulación no avanza más

> **Se cuelga, y sin decir nada** — es el error clásico de la primera semana, y el síntoma engaña: no hay error ni warning, el tiempo deja de avanzar y el objection nunca se baja. `get_next_item()` es un préstamo; `item_done()` es devolverlo.
...
- [x] Los dos reciben el mismo objeto: el segundo `set()` pisa al primero
...
- [x] Que se crea, corre y se tira: podés arrancar varias, una detrás de otra, sobre el mismo sequencer
- [ ] Que no puede tener campos `rand` ni constraints
- [ ] Que arranca sola al empezar la simulación, sin que nadie la llame

> **Se crea, corre y se tira** — un componente se construye una vez en `build_phase` y vive hasta el final. Por eso el estímulo no puede ser un componente: cambia test a test, y a veces dentro del mismo test. De ahí sale también que se pueda configurar entre el `create()` y el `start()`, como hace `full_seq.count = 200`.
...
- [x] Que el sequencer le dio el turno a esta sequence: nadie más le va a ganar el driver
- [ ] Que el `randomize()` ya se resolvió con las constraints de la clase
- [ ] Que el objection de la fase ya está levantado

> **Tenés el turno, todavía no entregaste nada** — y por eso el `randomize()` va *después*: es el último momento posible para elegir los valores, cuando ya sabés en qué estado está el DUT. Eso es la randomización tardía, y es donde se enganchan `pre_do()` y `mid_do()`.
...
- [ ] Corre igual: cuando hay `default_sequence`, el objection lo levanta el sequencer
- [x] La `main_phase` termina en t=0 y el test pasa con 0 errores sin haber mandado un solo estímulo
- [ ] La simulación se cuelga esperando un objection que nadie baja

> **Pasa en cero segundos, y miente** — una fase sin objection termina en cuanto arranca. Es el mismo tipo de bug que el `new()` que se come el override: no rompe, engaña. Y en una regresión de mil tests, el que pasa en cero segundos no lo mira nadie.
...
- [x] Para declarar a la hija sub-sequence de la madre: hereda su turno y su prioridad en la arbitración
- [ ] Para que la hija corra en un hilo aparte, en paralelo con la madre
- [ ] Para registrar a la hija en la factory con el nombre de la madre

> **Declara la relación madre-hija** — sin él, el sequencer trata a las dos como sequences independientes y compiten por el turno. Con él, la hija hereda el contexto. Y para el paralelo no alcanza con eso: hace falta un `fork` / `join` alrededor de los `start()`.
