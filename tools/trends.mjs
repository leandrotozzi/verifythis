// Genera res/trends/*.svg a partir de res/trends/data.json.
//
...
for (const fig of figures) {
  ...
  out.push(`<title>${esc(fig.title)} (${fig.year})</title>`);
  out.push(`<text x="${PAD}" y="46" fill="${TEXT}" font-size="34" font-weight="600">${esc(fig.title)}</text>`);
  out.push(`<text x="${PAD}" y="76" fill="${MUTED}" font-size="21">${esc(fig.scope ?? "IC/ASIC")}, ${fig.year}</text>`);
  ...
  out.push(`<text x="${PAD}" y="${y}" fill="${MUTED}" font-size="16">Datos: ${esc(fig.source)}. Gráfico propio.</text>`);
  out.push('</svg>');

  const file = path.join(DIR, fig.file);
